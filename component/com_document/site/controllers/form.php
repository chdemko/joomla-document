...
/**
 * @version		$Id: form.php 67 2010-12-14 14:16:21Z eexit $
 * @package		Document
...
/**
 * Form Controller of Document component
 */
class DocumentControllerForm extends JControllerForm
{
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		$userId		= $user->get('id');
		$asset		= 'com_document.document.'.$recordId;
	...
	public function edit()
	{
		...
		if ($id > 0) {
			...
			if ($item->checked_out == 0) {
				if (!$model->checkout($id)) {
					...
					$this->setRedirect('index.php?option=com_document&view=document&item_id='.$id, $message, 'error');
		...
		$redirect = 'index.php?option=com_document&view=form&layout=edit&id='.$id;
	...
	public function cancel()
	{
		...
		if ($recordId) {
			...
			if ($model->checkin($recordId) === false) {
				...
				$this->setRedirect('index.php?option=com_document&view=form&layout=edit&id='.$recordId, $message, 'error');
				return false;
			}
		}
	...
	public function save()
	{
		...
		if ($task == 'save2copy') {
			...
			if ($model->checkin() === false) {
				...
				$this->setRedirect('index.php?option=com_document&view=form&layout=edit', $message, 'error');
		...
		if ($data === false) {
			...
			$this->setRedirect(JRoute::_('index.php?option=com_document&view=form&layout=edit', false));
		...
		if ($recordId == 0) {
			$this->setMessage(JText::_('COM_DOCUMENT_SUBMIT_SAVE_SUCCESS'));
		} 
		else {
			$this->setMessage(JText::_('COM_DOCUMENT_SAVE_SUCCESS'));
		}
		...
		switch ($task)
		{
			case 'apply':
				...
				$recordId = $model->getState('document.id');
				$this->holdEditId($context, $recordId);
				$app->setUserState($context.'.data',	null);

				// Redirect back to the edit screen.
				$this->setRedirect(JRoute::_('index.php?option=com_document&view=form&layout=edit&id='.$recordId, false));
				break;
